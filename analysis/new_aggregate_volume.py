...
file_suffix = '.csv'
#path = './data/dataSets/training/'  # set the data directory
path = '../data/dataSets/testing_phase1/'

def avgVolume(in_file):

    out_suffix = '_20min_avg_volume_test'
    ...
    fw.writelines(','.join(['"tollgate_id"', '"time_window_month"', '"time_window_day"',  '"time_window_hour"',
                            '"time_window_minute"',  '"direction"', '"volume"']) + '\n')
        ...
        for tollgate_id in volumes[time_window_start]:
            for direction in volumes[time_window_start][tollgate_id]:
               out_line = ','.join(['"' + str(tollgate_id) + '"', 
			                     str(time_window_start.month), str(time_window_start.day),
                str(time_window_start.hour), str(time_window_start.minute), str(direction) ,str(volumes[time_window_start][tollgate_id][direction]),
                               ]) + '\n'
